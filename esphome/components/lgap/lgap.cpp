...
namespace esphome
{
  namespace lgap
  {
    ...
    void LGAP::dump_config()
    {
      ...
      ESP_LOGCONFIG(TAG, "  Power multiplier: %.2f", this->power_multiplier_);
      if (this->debug_ == true)
      {
    ...
    void LGAP::calculate_and_publish_power()
    {
      // Power estimation based on Byte 10 (load/operation rate) from all active zones
      
      if (this->devices_.size() == 0)
        return;

      // Step 1: Collect active zones and their load bytes
      std::vector<uint8_t> active_zone_loads;
      std::vector<LGAPDevice *> active_devices;
      uint32_t sum_loads = 0;
      uint8_t system_mode = 0;  // 0=cooling, 4=heating, determine from first active zone
      bool mode_determined = false;

      for (auto &device : this->devices_)
      {
        // Cast to LGAPHVACClimate to access specific methods
        auto *climate_device = static_cast<LGAPHVACClimate *>(device);
        
        if (climate_device->is_unit_on())
        {
          uint8_t load = climate_device->get_load_byte();
          active_zone_loads.push_back(load);
          active_devices.push_back(device);
          sum_loads += load;
          
          // Determine system mode from first active zone
          if (!mode_determined)
          {
            // Get mode from the climate object
            auto mode = climate_device->mode;
            if (mode == climate::CLIMATE_MODE_HEAT)
            {
              system_mode = 4;  // heating
            }
            else if (mode == climate::CLIMATE_MODE_COOL || mode == climate::CLIMATE_MODE_DRY || 
                     mode == climate::CLIMATE_MODE_FAN_ONLY || mode == climate::CLIMATE_MODE_HEAT_COOL)
            {
              system_mode = 0;  // cooling
            }
      ...
      // Step 2: If no zones are active or all load bytes are 0, report 0 power
      if (active_zone_loads.empty() || sum_loads == 0)
      {
        ESP_LOGV(TAG, "No active zones or zero load - power = 0 kW");
        
        // Publish 0 to total sensor
        if (this->total_power_sensor_ != nullptr)
        {
          this->total_power_sensor_->publish_state(0.0);
        }
        
        // Publish 0 to all zone sensors
        for (auto &device : this->devices_)
        {
          auto *climate_device = static_cast<LGAPHVACClimate *>(device);
      ...
      // Step 3: Compute system load (percent) - average of all normalized zone loads
      float system_load = 0.0;
      for (uint8_t load : active_zone_loads)
      {
        system_load += (float)load / 255.0;
      }
      system_load /= active_zone_loads.size();

      // Step 4: Choose rated max based on mode
      float rated_max_kw = (system_mode == 4) ? this->heating_max_power_ : this->cooling_max_power_;

      // Step 5: Calculate total power with multiplier
      float total_kw = system_load * rated_max_kw * this->power_multiplier_;
      
      // Cap at rated max
      if (total_kw > rated_max_kw * this->power_multiplier_)
      {
        total_kw = rated_max_kw * this->power_multiplier_;
      }

      ESP_LOGD(TAG, "Power calc: %d active zones, system_load=%.2f%%, mode=%s, total=%.2f kW", 
               active_zone_loads.size(), system_load * 100.0, 
               (system_mode == 4) ? "heating" : "cooling", total_kw);

      // Step 6: Publish total power
      if (this->total_power_sensor_ != nullptr)
      {
        this->total_power_sensor_->publish_state(total_kw);
      }

      // Step 7: Calculate and publish per-zone power
